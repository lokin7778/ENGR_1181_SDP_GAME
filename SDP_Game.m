...
%------------------------- HOME SCREEN CODE ------------------------------%
    ...
    while homeScreen
        ...
        if isequal(home_screen_keyboard_input, '1') || isequal(home_screen_keyboard_input, '2')
            ...
            stop(musicPlayer);   % stop the audio object
            clear sound
        end
        ...
        % the song will play agian and tic will reset. 
         if toc >= songLength
             play(musicPlayer); % play using the object
             tic;  % reset timer
         end
    end
...
%-------------------------- DIFFRENT MODES -------------------------------%
    ...
    if (isequal(home_screen_keyboard_input,'1'))
        ...
        % Loop stops after Level 11
        while level <= 11
            
            % --- LESSONS & QUIZZES (Levels 1 - 10) ---
            if level < 11
                % Load the scene text
            ...
            % --- LEVEL 11: MOVING CHARACTER BATTLE (STORY MODE ONLY) ---
            elseif level == 11
                current_scene = StoryMode(level);
                ...
                if score_moving == -1, story_exit = true; break; end % check for ESC
                ...
                level = level + 1; % Finish Story Mode
            end
        end
        
        if story_exit
            % User pressed ESC to exit Story Mode. 
            % 'flag' remains true, so it will loop back to Home Screen.
            clf; 
        else
            % End of Story Mode
    ...
    elseif (isequal(home_screen_keyboard_input,'2'))
        ...
        while exit_in_mode == 0
            ...
            % Randomly choose type 1 or 2 (Moving Character removed from Infinite)
            ...
            elseif question_type == 2 
                ...
                if exit_in_mode == 0
                    ...
                    % if they got the question right on their fist try...
                    % (Logic note: numTry starts at 0 for Fill-in-Blank in this loop)
                    if numTry == 0 
                        score = score + 1; 
                    else
                        score = score + .5; 
